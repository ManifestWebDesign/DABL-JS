...
Condition.prototype = {
	...
	_processCondition : function(left, right, operator, quote) {
		...
		isArray = false;
		if (right.constructor == Array || (right instanceof Query && right.getLimit() !== 1)) {
			isArray = true;
		}
		...
		if (isArray) {
			// BETWEEN
			if (right.constructor == Array && right.length == 2 && operator == Query.BETWEEN) {
			...
			// Handle empty arrays
			if (right.constructor == Array && !right) {
