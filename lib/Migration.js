...
//				});
	...
	if(migrations[1] && migrations[1].constructor === Object) {
		...
		if(options.constructor === Object && options.number !== null && typeof options != 'undefined')
...
Migration.setupSchema = function(force) {
	...
	if(Migration.count('SELECT COUNT(*) FROM schema_migrations') === 0) {
		sql = 'INSERT INTO schema_migrations (version) VALUES(0)';
		Migration.run(sql);
	}
	if(force && Migration.count('SELECT COUNT(*) FROM schema_migrations') === 1) {
...
Migration.currentSchemaVersion = function() {
	...
	return parseInt(Migration.run(sql)[0].version, 10);
};
...
Migration.modifyColumn = function(tableName, columnName, options) {
	...
	Migration.runTransaction(function() {
		var records = Migration.run('SELECT * FROM ' + tableName);
